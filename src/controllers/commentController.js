...
class commentController{
    ...
    async createComment(req,res){
        try{
            ...
            const newComment = await commentService.createComment(commentId,{nickname,content, password});
    ...
    async deleteComment(req,res){
        ...
        try{
            ...
            return res.status(500).sjon({message:'서버 오류'});
        }
    }
    ...
    async getComments(req,res){
        ...
        if(!commentId || isNaN(page)|| isNaN(pageSize)){
            return res.status(400).json({message:'잘못된 요청입니다.'});
        }
